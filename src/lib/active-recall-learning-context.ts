...
export type ActiveRecallLearningContext = {
  scope: {
    planId: string | null
  ...
  summary: {
    ...
    totalReviews: number
    accuracy: number | null
    masteryLayerDistribution: Record<number, number>
    recentSessions: {
      count: number
  ...
  slowResponseTopics: LearningTopicContext[]
  dueLoadByTopic: Array<{
    topic: string
    dueCount: number
    cardCount: number
  }>
  topicPerformance: LearningTopicContext[]
}
...
function topicStats(cards: ReviewCardRow[], nowIso: string): LearningTopicContext[] {
...
function weakTopicScore(topic: LearningTopicContext) {
...
function strongTopicScore(topic: LearningTopicContext) {
...
export async function buildActiveRecallLearningContext(
  supabase: SupabaseClient,
  userId: string,
  options: { planId?: string | null; documentId?: string | null }
): Promise<ActiveRecallLearningContext> {
  ...
  if (cardsError) throw new Error('Failed to fetch learning context cards')
  ...
  if (sessionsError) throw new Error('Failed to fetch learning context sessions')
  ...
  return {
    scope: { planId, documentId, generatedAt: nowIso },
    plan: plan ? {
      id: plan.id,
      title: plan.title,
