...
import { GraduationCap, CheckCircle2, Loader2, AlertCircle, ChevronRight } from 'lucide-react'
import { Button } from '@/components/ui'
import { cn } from '@/lib/utils'
import { useCourseStore, type GenerationProgress } from '@/lib/course-store'
import { useRouter } from 'next/navigation'

interface CourseGenerationLoadingProps {
  isOpen: boolean
  onClose?: () => void
  courseId?: string
}
...
const PHASE_INFO = {
  outline: {
    ...
    description: 'Reading your content and planning the perfect course structure...',
    icon: '📖',
    color: 'text-blue-500'
  },
  lessons: {
    title: 'Creating Lessons',
    description: 'Crafting bite-sized, ADHD-friendly lessons with visual diagrams...',
    icon: '✏️',
    color: 'text-teal-500'
  },
  quizzes: {
    title: 'Adding Quizzes',
    description: 'Generating quiz questions to help you master each topic...',
    icon: '🎯',
    color: 'text-purple-500'
  },
  verification: {
    title: 'Quality Check',
    description: 'AI is verifying course quality and ADHD-friendliness...',
    icon: '🔍',
    color: 'text-indigo-500'
  },
  finalizing: {
    title: 'Finishing Up',
    description: 'Polishing your course and getting everything ready...',
    icon: '✨',
    color: 'text-green-500'
  },
  complete: {
    title: 'Course Ready!',
    description: 'Your personalized course is ready to start learning!',
    icon: '🎉',
    color: 'text-green-500'
  },
  error: {
    title: 'Oops! Something Went Wrong',
    description: 'We encountered an issue generating your course.',
    icon: '⚠️',
    color: 'text-red-500'
  }
}

export const CourseGenerationLoading: React.FC<CourseGenerationLoadingProps> = ({
  isOpen,
  onClose,
  courseId
}) => {
  const router = useRouter()
  const { activeGeneration, activeCourse, stopPolling } = useCourseStore()

  const [canNavigateAway, setCanNavigateAway] = useState(false)
  const [timeElapsed, setTimeElapsed] = useState(0)

  // Enable navigation after 30 seconds
  useEffect(() => {
    if (isOpen) {
  ...
  useEffect(() => {
    if (isOpen && activeGeneration) {
      const interval = setInterval(() => {
        setTimeElapsed(Math.floor((Date.now() - activeGeneration.startTime) / 1000))
      }, 1000)
  ...
  useEffect(() => {
    if (activeCourse?.status === 'ready' && courseId) {
      // Wait 2 seconds to show completion animation
      setTimeout(() => {
        router.push(`/courses/${courseId}`)
        if (onClose) onClose()
      }, 2000)
    }
  }, [activeCourse?.status, courseId, router, onClose])

  const handleNavigateAway = () => {
    // Keep generation running in background, just close modal
    if (onClose) onClose()
  }
  ...
  if (!activeGeneration && !activeCourse?.generationProgress) return null

  // Always prefer activeCourse.generationProgress for the most up-to-date info
  const progress: GenerationProgress = activeCourse?.generationProgress || {
    phase: 'outline' as const,
    percentage: 0,
    currentStep: 'Starting generation...'
  }

  console.log('[GenerationModal] Progress:', progress) // Debug log
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className="relative bg-background border border-border/50 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden"
          ...
          >
            {/* Header */}
            <div className="relative p-10 border-b border-border/50 overflow-hidden">
              {/* Animated background gradient */}
              <motion.div
                className="absolute inset-0 bg-gradient-to-r from-teal-500/5 via-purple-500/5 to-blue-500/5"
                animate={{
              ...
              <div className="relative flex items-center gap-6 mb-6">
                ...
                <div className="flex-1">
                  ...
                  >
                    {activeGeneration?.courseTitle || 'Creating Your Course'}
                  </motion.h2>
            ...
            <div className="p-6 bg-muted/20 border-t border-border/50">
              {isComplete ? (
                ...
                >
                  ...
                  <span className="text-base font-semibold">Course Ready! Redirecting...</span>
                </motion.div>
              ) : isError ? (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button
                      ...
                      size="lg"
                      className="px-6 bg-gradient-to-r from-teal-500 to-purple-500 hover:from-teal-600 hover:to-purple-600"
                    >
                      Try Again
                    </Button>
                  </div>
                ...
                <div className="flex items-center justify-center">
                  <div className="text-center space-y-4">
                    <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>Generation running in background</span>
                    </div>
                    <Button
                      variant="outline"
                      size="lg"
